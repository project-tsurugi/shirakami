...
### 開始操作
* `tx_begin`
  + トランザクション開始を明示的に宣言する。トランザクションのモードをショート・ロング・読み込み専用を選択できる。本操作を実施せずにトランザクションに関するAPIを実行した場合、トランザクションを開始していない旨の警告を返却する。
...
- API 実行時に他 Tx による write preserve を観測しうり, 当該 API に読み込み作用が含まれるとき
  - ショートモード：自身のTxをアボートする。
  - ロングモード：それが自身のTxより高優先度なTxによるものだった場合、そのTxに対して前置することを考慮して操作を続行する。コミット時点で、相手による書き込みと自身の読み込みが衝突している懸念があると判断した場合、少なくともそのTxに対して前置する。
  - 読み込み専用モード：このモードでは並行Txにおいて最も古いトランザクションビューでトランザクションを開始しているため、write preserve の観測を考慮しない。
