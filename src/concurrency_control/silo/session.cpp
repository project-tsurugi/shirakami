...
Status session::check_delete_after_write(Record* rec_ptr) {
    ...
    if (get_write_set().get_for_batch()) {
        ...
        for (auto itr = cont.begin(); itr != cont.end(); ++itr) {
            ...
            if (ret != Status::WARN_ALREADY_EXISTS) { cont.erase(itr); }
            return ret;
        }
    } else {
...
void session::pwal(uint64_t commit_id, commit_property cp) {
    ...
    for (auto&& itr : read_set) {
        log_handle_.get_log_set().emplace_back(commit_id, OP_TYPE::SEARCH,
                                               &itr.get_rec_read().get_tuple());
        log_handle_.get_latest_log_header().add_checksum(
                log_handle_.get_log_set().back().compute_checksum()); // NOLINT
        log_handle_.get_latest_log_header().inc_log_rec_num();
    }
    ...
    if (log_handle_.get_log_set().size() > PARAM_PWAL_LOG_GCOMMIT_THRESHOLD ||
        cp == commit_property::WAIT_FOR_COMMIT) {
        // prepare write header
        log_handle_.get_latest_log_header()
                .compute_two_complement_of_checksum();
        ...
        auto write_batch = [this, &buffer, &offset](const void* ptr,
                                                    size_t size) {
            if (size > 4096) { // NOLINT
        ...
        for (auto&& itr : log_handle_.get_log_set()) {
            ...
            write_batch(static_cast<void*>(&itr),
            ...
            write_batch(static_cast<const void*>(value_view.data()),
                        value_size);
...
void session::regi_diff_upd_set(std::string_view const storage,
                                const tid_word& tid, OP_TYPE const op_type,
                                Record* const record,
...
void session::regi_diff_upd_seq_set(
        SequenceValue id, std::tuple<SequenceVersion, SequenceValue> ver_val) {
