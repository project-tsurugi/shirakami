...
Status check_empty_scan_range(const std::string_view l_key, const scan_endpoint l_end, // NOLINT(misc-use-internal-linkage)
...
static Status open_scan_body(Token const token, Storage storage, // NOLINT
...
Status open_scan(Token const token, Storage storage, // NOLINT
    ...
    shirakami_log_entry << "open_scan token: " << token << ", storage: " << storage
