...
inline std::shared_ptr<limestone::api::datastore> datastore_; // NOLINT
...
start_datastore(limestone::api::configuration const& conf) { // should not be "start_". change to "create_" or "setup_"
    datastore_ = std::make_shared<limestone::api::datastore>(conf); // NOLINT
}

[[maybe_unused]] static void set_datastore(const std::shared_ptr<limestone::api::datastore>& datastore) {
