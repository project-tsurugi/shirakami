...
#include "concurrency_control/wp/include/session_table.h"
...
void aggregate_diff_upd_set(
    ...
    for (auto&& table_elem : session_table::get_session_table()) {
        ...
        for (auto absorbed_storage = absorbed_set.begin();
             absorbed_storage != absorbed_set.end(); ++absorbed_storage) {
            if (get_checkpoint_thread_end() &&
                get_checkpoint_thread_end_force()) {
                return;
            }
#if defined(CPR_DIFF_HOPSCOTCH)
            for (auto map_elem = absorbed_storage.value().begin();
                 map_elem != absorbed_storage.value().end(); ++map_elem) {
                if ((aggregate_buf.find(absorbed_storage.key()) ==
                     aggregate_buf
                             .end()) || // not found storage in aggregate_buf
                    (aggregate_buf.find(absorbed_storage.key()) !=
                             aggregate_buf.end() &&
                     aggregate_buf[absorbed_storage.key()].find(
                             map_elem.key()) !=
                             aggregate_buf[absorbed_storage.key()]
                                     .end()) || // found storage but not found elem in aggregate_buf
                    std::get<cpr::cpr_local_handler::diff_timestamp_pos>(
                            aggregate_buf[absorbed_storage.key()]
                                         [map_elem.key()]) <
                            std::get<cpr_local_handler::diff_timestamp_pos>(
                                    map_elem.value())) {
                    aggregate_buf[absorbed_storage.key()][map_elem.key()] =
                            map_elem.value(); // merge
                }
            }
            absorbed_storage.value().clear();
            if (cpr_local_handler::reserve_num != 0) {
                absorbed_storage.value().reserve(
                        cpr_local_handler::reserve_num); // reserve for next
            }
#elif defined(CPR_DIFF_UM)
            for (auto map_elem = absorbed_storage->second.begin();
                 map_elem != absorbed_storage->second.end(); ++map_elem) {
                if ((aggregate_buf.find(absorbed_storage->first) ==
                     aggregate_buf
                             .end()) || // not found storage in aggregate_buf
                    (aggregate_buf.find(absorbed_storage->first) !=
                             aggregate_buf.end() &&
                     aggregate_buf[absorbed_storage->first].find(
                             map_elem->first) !=
                             aggregate_buf[absorbed_storage->first]
                                     .end()) || // found storage but not found elem in aggregate_buf
                    aggregate_buf[absorbed_storage->first][map_elem->first]
                                    .first < map_elem->second.first) {
                    aggregate_buf[absorbed_storage->first][map_elem->first] =
                            map_elem->second; // merge
                }
            }
            absorbed_storage->second.clear();
            absorbed_storage->second.reserve(
                    cpr_local_handler::reserve_num); // reserve for next
#endif
        }
        absorbed_set.clear(); // clean up.
        if (cpr_local_handler::reserve_num != 0) {
            absorbed_set.reserve(
                    cpr_local_handler::reserve_num); // reserve for next
        }
    }
}

void aggregate_diff_upd_seq_set(
        cpr_local_handler::diff_upd_seq_set_type& aggregate_buf) {
    phase_version pv = global_phase_version::get_gpv();
    auto index{pv.get_version() % 2 == 0 ? 0 : 1};
    for (auto&& table_elem : session_table::get_session_table()) {
        if (get_checkpoint_thread_end() && get_checkpoint_thread_end_force()) {
        ...
        for (auto map_elem = absorbed_map.begin();
             map_elem != absorbed_map.end(); ++map_elem) {
            if (get_checkpoint_thread_end() &&
                get_checkpoint_thread_end_force()) {
                return;
            }
            if (aggregate_buf.find(map_elem.key()) == aggregate_buf.end() ||
                std::get<0>(map_elem.value()) >
                        std::get<0>(aggregate_buf[map_elem.key()])) {
                aggregate_buf[map_elem.key()] = map_elem.value();
            }
        }
        absorbed_map.clear(); // clean up.
        absorbed_map.reserve(
                cpr_local_handler::reserve_num); // reserve for next
    }
}
...
cpr_local_handler::diff_upd_seq_set_type&
cpr_local_handler::get_diff_upd_seq_set() {
    version_type cv{get_version()};
    if ((cv % 2 == 0 && get_phase() == phase::REST) ||
        (cv % 2 == 1 && get_phase() != phase::REST)) {
...
void checkpoint_thread() {
    ...
    while (likely(!(get_checkpoint_thread_end() &&
                    get_checkpoint_thread_end_force()))) {
        ...
        if (get_checkpoint_thread_end() && !get_checkpoint_thread_end_force()) {
            if (session_table::is_empty_logs()) { break; }
...
void checkpointing() {
    ...
    if (aggregate_buf.size() + aggregate_buf_seq.size() != 0) {
        logf.open(cpr::get_checkpointing_path(), std::ios_base::out |
                                                         std::ios_base::binary |
                                                         std::ios_base::trunc);
        if (!logf.is_open()) {
     ...
     * memory is protected.
     */
        for (auto itr_storage = aggregate_buf.begin();
             itr_storage != aggregate_buf.end(); ++itr_storage) {
#if defined(CPR_DIFF_HOPSCOTCH)
            for (auto itr = itr_storage.value().begin();
                 itr != itr_storage.value().end(); ++itr) {
#elif defined(CPR_DIFF_UM)
            for (auto itr = itr_storage->second.begin();
                 itr != itr_storage->second.end(); ++itr) {
#endif
                if (get_checkpoint_thread_end() &&
                    get_checkpoint_thread_end_force()) {
                    return;
                ...
                if (std::get<cpr_local_handler::diff_is_delete_pos>(
                            itr.value())) {
                    l_recs.emplace_back(itr_storage.key(),
                                        std::string_view(itr.key()));
                } else {
                    l_recs.emplace_back(
                            itr_storage.key(), std::string_view(itr.key()),
                            std::get<cpr_local_handler::diff_value_pos>(
                                    itr.value()));
                }
            }
        }
    ...
    std::string fname{
            Log::get_kLogDirectory() + "/sst" +
            std::to_string(global_phase_version::get_gpv().get_version())};
    try {
...
void create_checkpoint() {
    ...
    auto process = [&l_recs_vc](std::size_t th_ct, Storage st) {
        ...
        for (auto&& elem : scan_res) {
            ...
            std::string val{};
            tuple.get_value(val);
            l_recs.emplace_back(st_view, key, val);
        }
    };
