 ...
 */
extern Status create_sequence(Token token, SequenceId* id);
 ...
 * @param[in] token the session token whose current transaction will be associated 
 ...
 */
extern Status read_sequence(Token token, SequenceId id,
                            SequenceVersion* version, SequenceValue* value);
 ...
 */
extern Status delete_sequence(Token token, SequenceId id);
