 ...
 * @details When it starts to use this system, in other words, it starts to build database, it must be executed first.
...
/**
 * @brief leave session
 * @details It return the objects which was got at enter function to kThreadTable.
 ...
 * @param[in] token the token retrieved by enter()
 * @param[in] storage the storage handle retrieved by register_storage() or get_storage()
 ...
 * @details This function executes tbegin(Token token) internaly, so it doesn't need to call tbegin(Token token).
 ...
 * @param len_val indicate the value length
 * @return Status::WARN_ALREADY_EXISTS The records whose key is the same as @a key exists in MTDB, so this function returned immediately.
 * @return Status::OK success
