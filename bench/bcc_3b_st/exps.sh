...
gen_graph_cc() {
  ...
  gnuplot -e 'tname="cc only"' ./../../../bench/bcc_3b_st/graph.plt
}

gen_graph_wp() {
  echo "start gen_graph_wp"
  gnuplot -e 'tname="wp"' ./../../../bench/bcc_3b_st/graph.plt
}
...
output_data_comment() {
  ...
  echo "#sudo perf stat -e cache-misses,cache-references -o ana.txt numactl -l ./bcc_3b_st -cpumhz $cpumhz -duration $duration -key_len $key_len -val_len $val_len -rec $rec -ol_ops $ol_ops -ol_rratio $rratio -ol_wp_wratio $ol_wp_wratio -ol_skew $skew -ol_thread sweep -bt_ops $rec -bt_rratio $rratio -bt_skew $skew -bt_thread $bt_thread" >>$result
...
exp() {
  ...
  for ((bt_thread = 1; bt_thread <= 16; bt_thread *= 2)); do
    for ol_wp_wratio in "${ol_wp_wratio_ary[@]}"; do
  ...
  for ((bt_thread = 1; bt_thread <= 16; bt_thread *= 2)); do
    ...
    for ol_wp_wratio in "${ol_wp_wratio_ary[@]}"; do
      echo "start loop ol_wp_wratio"
      for val_len in "${val_len_ary[@]}"; do
        echo "start loop val_len"
        for rratio in "${rratio_ary[@]}"; do
          echo "start loop rratio"
          decide_result_name $rratio $val_len $ol_wp_wratio $bt_thread
          rm -f $result

          output_data_comment
          for ((thread = 28; thread <= 112; thread += 28)); do
            ol_thread=$(echo "$thread - $bt_thread" | bc)
            echo "#sudo perf stat -e cache-misses,cache-references -o ana.txt numactl -l ./bcc_3b_st -cpumhz $cpumhz -duration $duration -key_len $key_len -val_len $val_len -rec $rec -ol_ops $ol_ops -ol_rratio $rratio -ol_wp_wratio $ol_wp_wratio -ol_skew $skew -ol_thread $ol_thread -bt_ops $rec -bt_rratio $rratio -bt_skew $skew -bt_thread $bt_thread"
            ...
            for ((i = 1; i <= epoch; ++i)); do
              ...
              date
              sudo perf stat -e cache-misses,cache-references -o ana.txt numactl -l ./bcc_3b_st -cpumhz $cpumhz -duration $duration -key_len $key_len -val_len $val_len -rec $rec -ol_ops $ol_ops -ol_rratio $rratio -ol_wp_wratio $ol_wp_wratio -ol_skew $skew -ol_thread $ol_thread -bt_ops $rec -bt_rratio $rratio -bt_skew $skew -bt_thread $bt_thread >exp.txt
...
cc_exp() {
  echo "start cc_exp"
  cd build_cc_release/bench/bcc_3b_st
  exp
  gen_graph_cc
  cd ../../../
}
...
wp_exp() {
  echo "start wp_exp"
  cd build_wp_release/bench/bcc_3b_st
  exp
  gen_graph_wp
  cd ../../../
}
