...
void check_flags() {
    ...
    if (FLAGS_ol_wp_wratio >= 0 && FLAGS_ol_wp_wratio <= 100) { // NOLINT
        std::cout << "FLAGS_ol_wp_wratio:\t" << FLAGS_ol_wp_wratio << std::endl;
    } else {
        LOG(FATAL) << "This is must be in the range 0 to 100.";
    }
