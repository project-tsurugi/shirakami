...
build() {
    ...
    if test $1 -eq 1; then
        cmake -DCMAKE_BUILD_TYPE=Release -DBUILD_CPR=OFF -DPARAM_READ_SET_CONT=0 ..
    elif test $1 -eq 2; then
        cmake -DCMAKE_BUILD_TYPE=Release -DBUILD_CPR=OFF -DPARAM_READ_SET_CONT=1 ..
    else
        echo "BAG $LINENO"
        exit
    fi
...
exp() {
    ...
    if test $1 -eq 1; then
        for tx_size in "${tx_sizes[@]}"; do
            echo "#sudo perf stat -e cache-misses,cache-references -o ana.txt numactl --interleave=all ./bcc_8 -d $duration -skew $skew -tx_size $tx_size"
            ...
            for ((i = 1; i <= epoch; ++i)); do
                ...
                date
                sudo perf stat -e cache-misses,cache-references -o ana.txt numactl --interleave=all ./bcc_8 -d $duration -skew $skew -tx_size $tx_size >exp.txt
            ...
            echo "$tx_size $avg_th $min_th $max_th $avg_mm $min_mm $max_mm" >>$result
        done
    elif test $1 -eq 2; then
        for skew in "${skews[@]}"; do
            echo "#sudo perf stat -e cache-misses,cache-references -o ana.txt numactl --interleave=all ./bcc_8 -d $duration -skew $skew -tx_size $tx_size"
            ...
            avg_th=$(echo "scale=4; $sum_th / $epoch" | bc | xargs printf %10.4f)
            avg_mm=$(echo "scale=4; $sum_mm / $epoch" | bc | xargs printf %10.4f)
            echo "$skew $avg_th $min_th $max_th $avg_mm $min_mm $max_mm" >>$result
        done
    else
        echo "BAG $LINENO"
        exit
