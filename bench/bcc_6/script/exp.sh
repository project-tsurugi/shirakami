...
exp() {
    ...
    trial_num=0
    for ((th = 28; th <= 112; th += 28)); do
        echo "#sudo perf stat -e cache-misses,cache-references -o ana.txt numactl -l ./bcc_6 -th $th -d $duration "
        ...
        for ((i = 1; i <= epoch; ++i)); do
            ...
            date
            sudo perf stat -e cache-misses,cache-references -o ana.txt numactl -l ./bcc_6 -th $th -d $duration >exp.txt
            tmp_ver=$(grep "gc_ver" ./exp.txt | awk '{print $2}')
            tmp_val=$(grep "gc_val" ./exp.txt | awk '{print $2}')
            sum_ver=$(echo "scale=4; $sum_ver + $tmp_ver" | bc | xargs printf %10.4f)
            sum_val=$(echo "scale=4; $sum_val + $tmp_val" | bc | xargs printf %10.4f)
            echo "tmp_ver: $tmp_ver, tmp_val: $tmp_val"
        ...
        avg_val=$(echo "scale=4; $sum_val / $epoch" | bc | xargs printf %10.4f)
        echo "$th $avg_ver $min_ver $max_ver $avg_val $min_val $max_val" >>$result
    done
    #return original pos
    cd ../../../bench/bcc_6
}
