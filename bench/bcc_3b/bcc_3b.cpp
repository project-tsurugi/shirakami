    ...
    RETRY: // for wp premature // NOLINT
        ...
        for (auto&& itr : opr_set) {
            ...
            if (itr.get_type() == OP_TYPE::SEARCH) {
                ...
                Storage ol_read_bt_st{};
                Storage bt_read_others_wp_st{};
                if (is_ol) {
                    ol_read_bt =
                ...
                for (;;) {
                    ...
                    if (is_ol) {
                        if (ol_read_bt) {
                            target_st = ol_read_bt_st;
                        } else {
                            target_st = storage;
                        }
                    } else {
                        if (bt_read_others_wp) {
                            target_st = bt_read_others_wp_st;
                        } else {
                            target_st = storage;
                        }
                    }
...
void init_google_logging() {
    google::InitGoogleLogging("shirakami-bench-bcc_3b");
...
int main(int argc, char* argv[]) try { // NOLINT
    ...
    log_dir.append("/shirakami_bench_bcc_3b");
