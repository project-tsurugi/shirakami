 ...
 * general option.
 */
DEFINE_uint32(bench_type, 0,
              "Do benchmarking for specific type. " // NOLINT
              "0 is insert benchmarking."
              "1 is batch insert benchmarking."
              "2 is update benchmarking."
              "3 is batch update benchmarking."
              "4 is creating and ingesting sst files. note that this bench is "
              "done by single thread.");
...
DEFINE_string(bt4_sst_path, "/tmp/rocksdb_test_sst",
              "The option for batch_type 4." // NOLINT
              "Where to make the SST file T.");
DEFINE_uint64(
        bt4_cont_res, 100 * 1000 * 1000,
        "The # of elements which is reserved for batch type 4."); // NOLINT
...
DEFINE_uint64(
        rocksdb_options_increase_parallelism, 16, // NOLINT
...
DEFINE_bool(rocksdb_options_direct_io_for_flush_and_compaction, false, // NOLINT
            "The value of "
            "ROCKSDB_NAMESPACE::Options.direct_io_for_flush_and_compaction.");
DEFINE_string(rocksdb_path, "/tmp/rocksdbtest",
              "Path to db for benchmarking."); // NOLINT
...
static void invoke_leader() {
    ...
    alignas(CACHE_LINE_SIZE) std::vector<std::uint64_t> res(
            FLAGS_thread); // NOLINT
...
void set_rocksdb_options(Options& options) {
    ...
    options.use_direct_io_for_flush_and_compaction =
            FLAGS_rocksdb_options_direct_io_for_flush_and_compaction;
}

int main(int argc, char* argv[]) { // NOLINT
    google::InitGoogleLogging("shirakami-bench-rocksdb_bench");
    gflags::SetUsageMessage(
            static_cast<const std::string&>("RocksDB benchmark")); // NOLINT
...
void bench_insert_process(std::uint64_t insert_end,
                          std::uint64_t& insert_cursor) {
    std::string_view key{reinterpret_cast<const char*>(&insert_cursor),
                         sizeof(insert_cursor)}; // NOLINT
...
void bench_batch_insert_process(std::uint64_t insert_end,
                                std::uint64_t& insert_cursor) {
    ...
    for (std::size_t i = 0; i < FLAGS_batch_write_num; ++i) {
        ...
        std::string_view key{reinterpret_cast<const char*>(&vec.at(i)),
                             sizeof(vec.at(i))}; // NOLINT
        std::string_view val{key};
        rocksdb::Slice rkey{key.data(), key.size()};
        rocksdb::Slice rval{val.data(), val.size()};
        batch.Put(rkey, rval);
...
void bench_update_process(std::uint64_t write_start, Xoroshiro128Plus& rnd) {
    ...
    std::string_view key{reinterpret_cast<const char*>(&kv),
                         sizeof(kv)}; // NOLINT
    std::string_view val{key};
    rocksdb::Slice rkey{key.data(), key.size()};
    rocksdb::Slice rval{val.data(), val.size()};
...
void bench_batch_update_process(std::uint64_t write_start,
                                Xoroshiro128Plus& rnd) {
    std::vector<std::uint64_t> vec;
    vec.reserve(FLAGS_batch_write_num);
    WriteBatch batch; // NOLINT
    for (std::size_t i = 0; i < FLAGS_batch_write_num; ++i) {
        vec.emplace_back(write_start +
                         (rnd.next() % (UINT64_MAX / FLAGS_thread)));
        std::string_view key{reinterpret_cast<const char*>(&vec.at(i)),
                             sizeof(vec.at(i))}; // NOLINT
        std::string_view val{key};
        rocksdb::Slice rkey{key.data(), key.size()};
...
void bench_create_ingest_sst(std::size_t const thid, std::uint64_t& sum,
                             const bool& quit) {
    Options options;
    ...
    std::cout << "Finish consume time[us]: "
              << (end_ts - begin_ts) / FLAGS_cpumhz / 1000;
    ...
    end_ts = rdtscp();
    std::cout << "Ingesting consume time[us]: "
              << (end_ts - begin_ts) / FLAGS_cpumhz / 1000;
}
