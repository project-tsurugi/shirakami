...
size_t
decideParallelBuildNumber(std::size_t record, std::size_t thread) 
{
...
parallel_build_mtdb(std::size_t thid, std::size_t start, std::size_t end, std::size_t len_val, std::vector<Tuple*> *insertedList) {
  ...
  for (uint64_t i = start; i <= end; ++i) {
    ...
    Tuple *tuple = new Tuple((char *)&keybs, sizeof(uint64_t), val.get(), len_val);
    Storage storage;
    insert(token, storage, (char *)&keybs, sizeof(uint64_t), val.get(), len_val);
...
void 
build_mtdb(std::size_t record, std::size_t thread, std::size_t len_val, std::vector<Tuple*> *insertedList)
{
  ...
  for (size_t i = 0; i < maxthread; ++i)
    thv.emplace_back(parallel_build_mtdb, i, i*(record / maxthread), (i+1)*(record / maxthread) - 1, len_val, &insertedList[i]);
...
void
delete_mtdb(std::size_t record, std::size_t thread, std::vector<Tuple*> *insertedList)
{
