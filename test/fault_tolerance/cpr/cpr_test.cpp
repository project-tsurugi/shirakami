...
TEST_F(cpr_test, cpr_recovery) { // NOLINT
    ...
    std::string k{"a"};
    std::string vb{};
    ASSERT_EQ(search_key(token, storage, k, vb), Status::OK);
    ASSERT_EQ(vb, "A"); // NOLINT
    std::string k2{"b"};
    ASSERT_EQ(search_key(token, storage, k2, vb), Status::OK);
    ASSERT_EQ(vb, "B");                   // NOLINT
    ASSERT_EQ(commit(token), Status::OK); // NOLINT
    ASSERT_EQ(delete_record(token, storage, k), Status::OK);
    ASSERT_EQ(commit(token), Status::OK); // NOLINT
    cpr::wait_next_checkpoint();
    ASSERT_EQ(leave(token), Status::OK);
    fin(false); // NOLINT
