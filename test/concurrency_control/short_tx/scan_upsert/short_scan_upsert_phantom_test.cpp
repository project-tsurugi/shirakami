...
class scan_upsert_phantom : public ::testing::Test { // NOLINT
public:
    static void call_once_f() {
        google::InitGoogleLogging("shirakami-test-concurrency_control-short_tx-"
                                  "scan_upsert-scan_upsert_phantom_test");
...
TEST_F(scan_upsert_phantom, scan_upsert_tx_find_phantom) { // NOLINT
    // prepare
    Storage st{};
    ...
    ASSERT_EQ(upsert(s1, st, "3", ""), Status::ERR_CC);
...
TEST_F(scan_upsert_phantom, scan_find_phantom_by_upsert) { // NOLINT
    // prepare
    Storage st{};
    ASSERT_EQ(Status::OK, create_storage("", st));
    Token s1{};
