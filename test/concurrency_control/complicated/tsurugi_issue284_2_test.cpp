...
class tsurugi_issue284_2 : public ::testing::TestWithParam<std::tuple<transaction_options::transaction_type, bool>> { // NOLINT
public:
    static void call_once_f() {
        google::InitGoogleLogging("shirakami-test-concurrency_control-"
                                  "complicated-tsurugi_issue284_2");
...
INSTANTIATE_TEST_SUITE_P(TwoBoolParam, tsurugi_issue284_2,
    ::testing::Values(
        std::make_tuple(transaction_options::transaction_type::LONG, true),
        std::make_tuple(transaction_options::transaction_type::LONG, false),
        std::make_tuple(transaction_options::transaction_type::READ_ONLY, true),
        std::make_tuple(transaction_options::transaction_type::READ_ONLY, false)
    ));


TEST_P(tsurugi_issue284_2, DISABLED_read_from_scan_must_not_read_absent_record) { // NOLINT
    ...
    // LTX/RTX: open_scan
    ScanHandle scanh;
    ASSERT_OK(open_scan(l, st, "", scan_endpoint::INF, "", scan_endpoint::INF, scanh));

    do {
        std::string key;
        std::string val;
        if (auto rc = read_key_from_scan(l, scanh, key); rc != Status::OK) {
            VLOG(10) << "read_key_from_scan rc:" << rc;
            continue;
        }
        if (auto rc = read_value_from_scan(l, scanh, val); rc != Status::OK) {
