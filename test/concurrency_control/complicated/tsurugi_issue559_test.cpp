...
class tsurugi_issue559_test : public ::testing::TestWithParam<bool> {
public:
    static void call_once_f() {
        google::InitGoogleLogging("shirakami-test-concurrency_control-"
                                  "complicated-tsurugi_issue559_test");
...
INSTANTIATE_TEST_SUITE_P(has_initialization, tsurugi_issue559_test,
                         ::testing::Values(false, true));

TEST_P(tsurugi_issue559_test, DISABLED_simple) { // NOLINT
    bool search_and_upsert = GetParam();
    /**
    ...
    */
    ...
    auto put = [&](int key, Token t) {
        ...
        if (search_and_upsert) {
            std::string buf{};
            ASSERT_EQ(search_key(t, st, keystr, buf), Status::WARN_NOT_FOUND);
