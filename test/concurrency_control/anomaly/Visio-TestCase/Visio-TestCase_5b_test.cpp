...
public:
    static void call_once_f() {
        google::InitGoogleLogging(
                ...
                "anomaly-Visio-TestCase-Visio-Test'Case_5b_test");
...
TEST_P(Visio_TestCase, test_1) { // NOLINT
    ...
    if (t1_type == transaction_type::LONG || t2_type == transaction_type::LONG || t3_type == transaction_type::LONG || t4_type == transaction_type::LONG) { GTEST_SKIP() << "LONG is not supported"; }
    ...
    if (t4_type == transaction_type::SHORT &&
        ...
        ASSERT_EQ(buf, "0");
        auto t4_rc = commit(t4, cb4);
        if (t2_type == transaction_type::LONG) { ASSERT_FALSE(t4_rc); }
    }
