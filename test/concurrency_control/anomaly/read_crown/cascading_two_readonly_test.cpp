...
class cascading_two_readonly_test : public ::testing::Test { // NOLINT
public:
    static void call_once_f() {
        google::InitGoogleLogging("shirakami-test-concurrency_control-anomaly-"
                                  "read_crown-cascading_two_readonly_test");
...
TEST_F(cascading_two_readonly_test, all) { // NOLINT
                                           // create table
    // ==========
    // prepare
    Storage stu{};
    ...
    ASSERT_EQ(create_storage("w", stw), Status::OK);
    ...
    ASSERT_EQ(Status::OK, search_key(s.at(2), stw, w, buf));
    ASSERT_EQ(buf, var.at(0));
    ...
    ASSERT_EQ(Status::OK,
              tx_begin({s.at(6), transaction_options::transaction_type::LONG}));
    wait_epoch_update();
    ...
    ASSERT_EQ(Status::OK, search_key(s.at(0), stw, w, buf));
    ASSERT_EQ(buf, var.at(0));
