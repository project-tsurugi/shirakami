...
class overlapped_4tx_with_epoch_cycled_3_test
    : public ::testing::Test { // NOLINT
public:
    static void call_once_f() {
        google::InitGoogleLogging(
                "shirakami-test-concurrency_control-anomaly-"
                "write_crown-overlapped_4tx_with_epoch_cycled_3_test");
...
TEST_F(overlapped_4tx_with_epoch_cycled_3_test, all) { // NOLINT
                                                       // create table
    // ==========
    // prepare
    Storage sta{};
    ...
    // note: boundary can't decide till finishing t2
    ...
    ASSERT_EQ(Status::OK, upsert(s.at(1), stb, b, v.at(1)));
    ASSERT_EQ(Status::OK, commit(s.at(1)));
    // t2 must wait for t1 to decide boundary
    ASSERT_EQ(Status::OK, commit(s.at(2)));
    ...
    ASSERT_EQ(Status::OK, upsert(s.at(2), stb, b, v.at(2)));
    ASSERT_EQ(Status::OK, commit(s.at(2)));
    // t3 must wait for t2 to decide boundary
    ASSERT_EQ(Status::OK, commit(s.at(3)));
    // t4 must wait for t3 to decide boundary
    ASSERT_EQ(Status::OK, commit(s.at(4)));
    wait_epoch_update();
    ASSERT_EQ(Status::OK, upsert(s.at(1), sty, y, v.at(1)));
    ASSERT_EQ(Status::OK, upsert(s.at(1), stb, b, v.at(1)));
