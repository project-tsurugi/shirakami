...
class start_with_occ_cascading_test : public ::testing::Test { // NOLINT
public:
    static void call_once_f() {
        google::InitGoogleLogging("shirakami-test-concurrency_control-anomaly-"
                                  "write_crown-start_with_occ_cascading_test");
...
TEST_F(start_with_occ_cascading_test, all) { // NOLINT
                                             // create table
    // ==========
    // prepare
    Storage sta{};
    ...
    ASSERT_EQ(Status::OK, upsert(s.at(5), stb, b, v.at(5)));
