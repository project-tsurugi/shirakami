...
class overlapped_4tx_with_epoch_uncycled_2_test
    : public ::testing::Test { // NOLINT
public:
    static void call_once_f() {
        google::InitGoogleLogging(
                "shirakami-test-concurrency_control-anomaly-"
                "write_crown-overlapped_4tx_with_epoch_uncycled_2_test");
...
TEST_F(overlapped_4tx_with_epoch_uncycled_2_test, all) { // NOLINT
                                                         // create table
    // ==========
    // prepare
    Storage sta{};
    ...
    ASSERT_EQ(Status::OK, upsert(s.at(3), stb, b, v.at(3)));
    ASSERT_EQ(Status::OK, commit(s.at(3)));
    ASSERT_EQ(Status::OK, commit(s.at(4))); // 4 must wait for boundary decision
    wait_epoch_update();
