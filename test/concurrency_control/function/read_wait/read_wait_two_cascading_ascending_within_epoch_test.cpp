...
class read_wait_two_cascading_ascending_within_epoch_test
    : public ::testing::Test { // NOLINT
public:
    static void call_once_f() {
        google::InitGoogleLogging("shirakami-test-concurrency_control-function-"
                                  "read_wait-read_wait_two_cascading_ascending_"
                                  "within_epoch_test");
...
TEST_F(read_wait_two_cascading_ascending_within_epoch_test, // NOLINT
       two_cascading_ascending_within_epoch) {              // NOLINT
                                                            // create table
    // ==========
    // prepare
    Storage sta{};
    ...
    ASSERT_EQ(Status::OK, tx_begin({s.at(5),
                                    transaction_options::transaction_type::LONG,
                                    {stx}}));
    epoch::set_perm_to_proc(epoch::ptp_init_val); // unlocked
    wait_epoch_update();
