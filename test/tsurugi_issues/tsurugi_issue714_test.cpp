...
TEST_F(tsurugi_issue714_test, // NOLINT
    ...
    */
    ...
    ASSERT_OK(tx_begin({t2, transaction_type::SHORT}));
    ASSERT_EQ(Status::WARN_NOT_FOUND,
              open_scan(t2, st_c, "", scan_endpoint::INF, "",
                        scan_endpoint::INF, shd));
    ASSERT_OK(insert(t2, st_a, "1", ""));
    ASSERT_OK(commit(t2));
    // t1 < t2
    ...
    ltx_begin_wait(t3);
    ASSERT_EQ(Status::WARN_NOT_FOUND,
              open_scan(t3, st_b, "", scan_endpoint::INF, "",
                        scan_endpoint::INF, shd));
    // t3 < t1 < t2
    ASSERT_OK(insert(t3, st_c, "1", ""));
    // t3 write c will break committed t2 read c *1
...
TEST_F(tsurugi_issue714_test, shortx_positive) { // NOLINT
    // prepare
    Storage st{};
    ...
    ASSERT_EQ(static_cast<session*>(t1)->get_result_info().get_reason_code(),
...
TEST_F(tsurugi_issue714_test, shortx_positive_longkey) { // NOLINT
    // prepare
    Storage st{};
    ASSERT_OK(create_storage("", st));
    Token t1{};
    ...
    // test
    std::string_view k{"123456781234567812345678"};
    ASSERT_OK(tx_begin({t1, transaction_type::SHORT}));
    ASSERT_OK(tx_begin({t2, transaction_type::SHORT}));
    std::string buf{};
    ASSERT_EQ(Status::WARN_NOT_FOUND, search_key(t1, st, k, buf));
    ASSERT_OK(insert(t2, st, k, ""));
    ASSERT_OK(commit(t2));
    ASSERT_EQ(Status::ERR_CC, commit(t1));
    ASSERT_EQ(static_cast<session*>(t1)->get_result_info().get_reason_code(),
              reason_code::CC_OCC_PHANTOM_AVOIDANCE);
...
TEST_F(tsurugi_issue714_test, shortx_positive_longkey2) { // NOLINT
    std::string_view k1{"123456781234567812345678"};
    std::string_view k2{"1234567812345678123456XX"};
    // prepare
    Storage st{};
    ASSERT_OK(create_storage("", st));
    Token t1{};
    ...
    ASSERT_OK(tx_begin({t2, transaction_type::SHORT}));
    std::string buf{};
    ASSERT_EQ(Status::WARN_NOT_FOUND, search_key(t1, st, k2, buf));
    ASSERT_OK(insert(t2, st, k2, ""));
    ASSERT_OK(commit(t2));
    ASSERT_EQ(Status::ERR_CC, commit(t1));
    ASSERT_EQ(static_cast<session*>(t1)->get_result_info().get_reason_code(),
              reason_code::CC_OCC_PHANTOM_AVOIDANCE);
...
TEST_F(tsurugi_issue714_test, shortx_false_positive) { // NOLINT
    // prepare
    Storage st{};
    ASSERT_OK(create_storage("", st));
    Token t1{};
